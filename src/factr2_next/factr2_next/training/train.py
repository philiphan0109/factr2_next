...
def normalized_model_cfg(model_cfg):
    ...
    if model_cfg["type"] in ("gru", "lstm"):
        model_cfg["state_mode"] = str(
            model_cfg.get("state_mode", "stateless")
        ).lower()
    return model_cfg
...
def default_config_path():
    try:
        ...
        return (
            Path(get_package_share_directory("factr2_next"))
            / "config"
            / "train.yaml"
        )
    except Exception:
        return Path(__file__).resolve().parents[1] / "config" / "train.yaml"
