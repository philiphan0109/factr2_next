...
class BaseTeleopController(Node, ABC):
    def __init__(self):
        ...
        config_path = Path(self.config_file_name).expanduser()
        if not config_path.is_absolute():
            config_path = Path(self.share_dir) / "configs" / config_path
        ...
        self.gravity_compensation_enabled = self.config["shared"]["controller"]["gravity_comp"]["enable"]
    ...
    def find_ttyusb(self, port_name):
        ...
        """
        base_path = Path("/dev/serial/by-id")
        full_path = Path(port_name)
        if not full_path.is_absolute():
            full_path = base_path / full_path

        if not os.path.exists(full_path):
            raise FileNotFoundError(
                f"Port '{port_name}' does not exist. Check {base_path}."
            )
        try:
    ...
    def _prepare_dynamixels(self):
        ...
        self.dynamixel_port = str(
            Path("/dev/serial/by-id") / self.config[self.name]["dynamixel"]["port"]
        )
        ttyUSBx = self.find_ttyusb(self.dynamixel_port)
        self._warn_if_latency_timer_high(ttyUSBx)
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Port {self.dynamixel_port} not found. "
                "Set the Dynamixel port in piper_gellos.yaml."
            ) from exc
    ...
    def _warn_if_latency_timer_high(self, ttyusb):
        path = Path("/sys/bus/usb-serial/devices") / ttyusb / "latency_timer"
        try:
            latency = int(path.read_text().strip())
        except FileNotFoundError:
            self.get_logger().warn(f"Could not find latency timer: {path}")
            return
        except Exception as e:
            self.get_logger().warn(f"Could not read latency timer {path}: {e}")
            return

        if latency != 1:
            self.get_logger().warn(
                f"{ttyusb} latency_timer is {latency}; high-rate teleop may suffer. "
                f"To set it to 1, run: echo 1 | sudo tee {path}"
            )
    ...
    def shut_down(self):
        ...
        try:
            ...
            try:
                self.get_logger().error(f"Torque disable failed: {e}")
            except Exception:
                print(f"Torque disable failed: {e}")
    ...
    def get_joint_limit_barrier_repulsion_torque(self, arm_joint_pos, arm_joint_vel, gripper_joint_pos, gripper_joint_vel):
        """
        computes the repulsive torques to prevent the leader arm joints and gripper from exceeding the physical joint limits of the follower arm
        """
    ...
    def set_leader_joint_torque(self, arm_torque, gripper_torque):
        """
        applies a specified torque to the leader arm joints and gripper
        """
    ...
    def gravity_compensation(self, arm_joint_pos, arm_joint_vel):
        ...
        uses the Recursive Newton-Euler Algorithm (RNEA) provided in the Pinocchio library
        """
        self.tau_gravity = pin.rnea(
            self.pin_model,
            self.pin_data,
            arm_joint_pos,
            arm_joint_vel,
    ...
    def compute_gripper_feedback_torque(self, leader_gripper_pos, leader_gripper_vel):
        """
        this method processes feedback signals for the gripper and returns the force-feedback input for the leader gripper
